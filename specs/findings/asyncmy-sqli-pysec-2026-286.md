# Finding: asyncmy SQL-injection (PYSEC-2026-286) — Critical, FIXED upstream

**Opened:** 2026-07-04 (from pip-audit during the daily state-tax audit gate)
**Status:** **RESOLVED — MERGED TO MAIN 2026-09-09** (commit `c4b82f0`).
Fixed upstream in **asyncmy 0.2.12**; constraint floor `^0.2.12`, lock resolves
**0.2.14**. CI green on all jobs, including `Tests (mariadb)`.
...
- 2026-09-09: **MERGED.** Branch `deps/asyncmy-fix-0.2.12` merged to main as `c4b82f0` on Eric's go-ahead; pushed and **CI green on every job** (Lint and type check, Tests (postgres), Tests (mariadb); DCO check skipped as it only runs on PRs). Local gate re-run post-merge: ruff format 216 files clean, ruff check clean, mypy clean (136 files), 1590 unit tests passed / 4 skipped. pip-audit evidence strengthened with a **controlled test** — an isolated `pip-audit --no-deps -r` on a pinned `asyncmy==0.2.11` still reports PYSEC-2026-286 (empty Fix Versions), while the same audit on `asyncmy==0.2.14` returns "No known vulnerabilities found", so the clean result is a real signal and not just the package being absent from the dev venv. **Correction to the 2026-08-27 caveat:** that entry said the bump was "verified by the Python-side gate only, not against a live MariaDB server" — that is wrong. CI has a `Tests (mariadb)` matrix job which installed asyncmy 0.2.14 and ran the suite against a live MariaDB service on :3306, so 0.2.12's escape_dict removal and its bundled protocol rewrite ARE exercised against a real server. OSV remains un-updated (`last_affected: 0.2.11`, no `fixed` event, unmodified since 2026-07-01). aiomysql latest 0.3.2 (2025-10-22, unchanged; maintained; only advisory PYSEC-2026-1110 / GHSA-r397-ff8c-wv2g fixed in 0.3.0, so 0.3.2 is unaffected).
