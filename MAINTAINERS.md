...
| New Mexico | NM | _vacant — see issue tracker_ | _post-v0.12 (non-SST tier-0 -> tier-1; **GROSS RECEIPTS TAX (GRT) -- NOT a sales tax**, imposed on the SELLER's gross receipts under NMSA 1978 Chapter 7 Article 9, modeled as sales tax for OpenSalesTax v1 API-shape compatibility (mirrors HI/GET approach). Statewide 4.875% per NMSA 7-9-4 effective 2023-07-01 under HB 163 of 2022 (contingent further reduction to 4.500% on revenue triggers not yet met). State-portion only -- 33 counties + ~106 municipalities each with local-option GRT increments deferred to per-county data ingestion (TRD location-code CSV; mirrors CO/LA/SC/MS/MO/NV precedent). Combined rates ~5.5%-9.5% across NM addresses; v1 caller under-collects by 1-4 percentage points. **Groceries effectively exempt at BOTH state AND local layers** via NMSA 7-9-92 deduction + state hold-harmless distribution under NMSA 7-1-6.46/.47 (unlike GA where state exempts but locals tax). Prescription drugs / oxygen / medical cannabis exempt per NMSA 7-9-73.2. Digital goods taxable since 2019-07-01 per HB 6 of 2019. **Annual Back-to-School Tax-Free Holiday** per NMSA 7-9-95 as amended by HB 218 of 2025 (moved from "first Friday in August" to "**last Friday in July through following Sunday**"); 2026 = July 31 - August 2; six scopes (clothing<$100, bookbags<$100, computers<=$1000, computer hardware<=$500, calculators<$200, school supplies<$30) each emitted as a separate HolidayWindow. Holiday is a SELLER-elected DEDUCTION not an automatic exemption per TRD FYI-203 -- callers should treat zero-tax as the eligible-for-exemption answer.)_ |
...
| Puerto Rico | PR | _vacant — see issue tracker_ | _Phase 8 (US-territory tier-0 -> tier-1; **PR is a US territory, not a US state** -- IVU (Impuesto sobre Ventas y Uso) administered by the Departamento de Hacienda de Puerto Rico under the Codigo de Rentas Internas de Puerto Rico (13 L.P.R.A. §§ 32001 et seq.); combined rate **11.5% = 10.5% state per 13 L.P.R.A. § 32021 + 1.0% municipal per 13 L.P.R.A. § 32024**, encoded as a single combined territory-level RateRow because the consumer-facing rate is uniform across all 78 PR municipalities (no per-municipality variation; centralized Hacienda collection); effective 2015-07-01 under Act No. 72 of 2015; **non-SST** (US territories are not SST-eligible); groceries (unprepared food and ingredients) EXEMPT per 13 L.P.R.A. § 32023 with prepared food / restaurant meals / candy / soft drinks / dietary supplements remaining taxable; prescription drugs EXEMPT with OTC medications taxable; digital goods taxable; SEPARATE 4% Special IVU on B2B services per 13 L.P.R.A. § 32022 OUT OF SCOPE for v1 retail engine; "Dias sin IVU" annual back-to-school holiday is enacted ad hoc by the legislature each year and the 2026 dates were NOT YET announced by Hacienda at module ship -- ``holidays_for(year)`` returns empty until officially announced; primary statutory sources are codified in **Spanish** (Codigo de Rentas Internas) with English translations via L.P.R.A. as secondary references)_ |
