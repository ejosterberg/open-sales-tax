...
async def test_states_marks_unsupported_states_tier_0(client: AsyncClient) -> None:
    ...
    CT/DC/SC/VA in v0.6; CO/ID/LA/MO/MS in v0.7; ME in v0.12; HI in
    Phase 6 Batch C (GET model encoded as sales tax for API purposes).
    AL and NM remain tier 0 until their state modules ship. (NM uses
    the Gross Receipts Tax model and waits on a separate non-sales-tax
    abstraction.)
    """
    response = await client.get("/v1/states")
    states_by_abbrev = {s["abbrev"]: s for s in response.json()["states"]}
...
async def test_phase_3_non_sst_states_are_tier_1(client: AsyncClient) -> None:
    ...
    ME (v0.12, non-SST, no local tax), HI (Phase 6 Batch C, non-SST,
    GET model encoded as sales tax with per-county surcharges deferred):
    all tier 1 non-SST.
    """
    response = await client.get("/v1/states")
    states_by_abbrev = {s["abbrev"]: s for s in response.json()["states"]}
