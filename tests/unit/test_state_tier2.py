...
(OH), Oklahoma (OK), and Tennessee (TN) were promoted to tier 1
in v0.8 - v0.11; they now live in their dedicated modules under
``opensalestax/states/``.
"""
...
# All 7 tier-2 states should be present (AR+GA+IA+IN+KS+KY+MI+NC+ND+NE+NJ+NV+OH+OK+TN were promoted to tier 1 in v0.8 - v0.11).
...
def test_phase_1_states_all_registered() -> None:
    ...
    # 7 tier-2 + MN+WI+AR+GA+IA+IN+KS+KY+MI+NC+ND+NE+NJ+NV+OH+OK+TN (tier 1 SST) + AK, DE, MT, NH, OR (no-tax)
